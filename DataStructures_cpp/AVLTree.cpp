class AVLTree{
    private:
        ...
        int BalanceFactor(TreeNode<int>* node) {
            if (node == nullptr) return 0;

            int leftHeight = (node->GetLeft() != nullptr) ? Height(node->GetLeft()->GetData()) : -1;
            int rightHeight = (node->GetRight() != nullptr) ? Height(node->GetRight()->GetData()) : -1;

            return leftHeight - rightHeight;
        }
    
        void RightRotation(TreeNode<int>* x){
            TreeNode<int>* y = x->GetLeft();
            TreeNode<int>* B = y->GetRight();
            TreeNode<int>* u = x->GetParent();
        ...
        void LeftRotation(TreeNode<int>* x){
            TreeNode<int>* y = x->GetRight();
            TreeNode<int>* B = y->GetLeft();
            TreeNode<int>* u = x->GetParent();
    ...
    public:
        ...
        void Insert(int key){
            ...
            TreeNode<int>* x = newItem;
            while(x != nullptr){
                int balance = BalanceFactor(x);

                if(balance > 1){ // Left
                    if(BalanceFactor(x->GetLeft()) >= 0){ // Left-Left Case
                        RightRotation(x);
                    }
                    ...
                    if(BalanceFactor(x->GetRight()) <= 0){ // Right-Right Case
                        LeftRotation(x);
                    }
        ...
        void Delete(int key){
            ...
            TreeNode<int>* current = Search(key);
            TreeNode<int>* parentToBalance = current->GetParent();
            ...
            if(current->GetRight() == nullptr && current->GetLeft() == nullptr){ // Case 1: No children
                ...
                TreeNode<int>* successor = Successor(current->GetData());
            ...
            // Balancing the tree
            TreeNode<int>* x = parentToBalance;
            while(x != nullptr){
                int balance = BalanceFactor(x);
